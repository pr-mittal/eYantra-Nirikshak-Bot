...
# Filename:		 task_3.py
# Functions:		init_setupsetAngles,coordinateTransform,control_logic
# Global variables: outMax,outMin,SampleTime
...
# outMax: max angle of servo that is allowed
outMax=60
...
def setAngles(client_id,revolute_handle,Output):
    ...
    Purpose:
    ---
	set angles int he sevos ,all at the same time
    Input Arguments:
    ---
	'client_id':[number]
		id of client
	'revolute_handle':[list]
		list of all the 8 revolute handles for this table
	'Output':[list]
		The angles values
    Returns:
    ---
    ...
    Example call:
    ---
    setAngles(client_id,revolute_handle,Output)
    '''
...
def init_setup(rec_client_id,table_number):
	...
	1. Get all the required handles from the CoppeliaSim scene.
	...
	'table_number':[int]
		the table nummber for which this function is called
	Returns:
	---
	'revolute_handle':[list]
		the list of all the revolute joint handles for this table
	'vision_sensor_handle':[list]
		the vision sensor handle
	Example call:
	---
	revolute_handle,vision_sensor_handle=init_setup(rec_client_id,table_number)
...
def coordinateTransform(xy):
    ...
    Purpose:
    ---
	Transform of the coordinates from the x y system to diagonal sytem as servos are placed diagonally

    Input Arguments:
    ---
	'xy':[list]
		the x y coornates of the ball
    Returns:
    ---
	'transformed':[np.array]
		the tranformed coordinates taking x qand y axis diagonally
    Example call:
    ---
...
def control_logic(setpoint,client_id,center_x,center_y,lastInput,lastTime,lastOutput,summation,kp,ki,kd):
	"""
	Purpose:
	---
	...
	---
	'setpoint':[list]
		The destination ball is moving towards
	'client_id':[int]
		the id of client
	'center_x':[int]
		the x coordinate of ball
	'center_y':[int]
		the y coordinate of ball
	'lastInput':[np.array]
		the last input which was received by this function, after previous iteration , used to calculate i and d term of pid
	'lastTime':[float]
		the last time the simulation ran, after previous iteration , used to calculate i and d term of pid
	'lastOutput':[np.array]
		the last Output which was calculated by this function
	'summation':[np.array]
		the summation of error encountered , i.e. integral term in pid after previus iteration
	'kp':[np.array]
		the propotional variable
	'ki':[np.array]
	...
	---
	'Input':[ numpy array]
		the new Input calulated by this function
	'now':[ float]
		the latest time 
	'Output':[ numpy array]
		the output calculated 
	'summation':[ numpy array]
		the summations of error , for integral term calculated
	Example call:
	---
	Input,now,Output,summation=control_logic(setpoint,client_id,center_x,center_y,lastInput,lastTime,lastOutput,summation,kp,ki,kd)
