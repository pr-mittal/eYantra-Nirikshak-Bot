...
def input_array(frame,roi,thresh):
    ...
    lower_blue=np.array([max(0,blue_min[0]-10),max(0,blue_min[1]-10),max(0,blue_min[2]-10)])
    upper_blue=np.array([min(255,blue_max[0]+10),min(255,blue_max[1]+10),min(255,blue_max[2]+10)])
    #mask
...
while True:
    ...
    if (count_blue==1) or (count_green_white==0):
        ...
        while True:
            ...
            #going to the first non corner white box
            count_blue=np.where(green_matrix==5)
            count_blue=list(zip(count_blue[0],count_blue[1]))

            for x in white_box:
                for y in count_green:
                    if(x!=y) and (x!=flag_leave):
                        done=True
                        corners,current_pos,_=aruco_marker(frame,roi)
            ...
            if(count_loop==4):
                ...
                count_green=np.where(green_matrix==6)
                count_green=list(zip(count_green[0],count_green[1]))
                #preparing for left out green box where white box is still there
                done=1
                for y in white_box:
