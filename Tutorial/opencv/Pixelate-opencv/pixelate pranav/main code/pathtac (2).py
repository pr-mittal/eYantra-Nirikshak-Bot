#while checking hidden colour check if neighbour us white or not
#give values for blue
#before going to destination change destination no. from 0 to any arbitary constant,after reaching check the shape and change no.according to shape.
#make cases if somewhere input does not come
#check green mask
#importing liraries
import cv2
import numpy as np
...
def djikstras(current_pos,final_pos,input_array,extension=0,min_colour=1):
    ...
    def dspa(a,b,c,d,e):
        if arr[a+1][b+1]!=0 and a!=-1 and b!=-1 and a!=5 and b!=5:
            if d_arr[a][b]>c:
...
def pathFolllower(frame,path,current_pos,roi=[0,0,np.inf,np.inf]):
    ...
    for i in range(1,len(path)):
        while True:
             ...
             centroid=[((path[i]//5)*col/5+((path[i]//5 +1)*col/5))/2,((path[i]%5)*row/5+((path[i]%5+1)*row/5))/2]
...
def input_array(frame,roi,thresh):
    ...
    lower_blue=np.array([max(0,blue_min[0]-15),max(0,blue_min[1]-15),max(0,blue_min[2]-15)])
    upper_blue=np.array([min(255,blue_max[0]+15),min(255,blue_max[1]+15),min(255,blue_max[2]+15)])
    #mask
    ...
    check value of 0.05
    '''
            approx_red=cv2.approxPolyDP(cnt,0.01*cv2.arcLength(cnt,True),True)
            ...
            if(M["m00"]!=0):
                ...
                if(row_red<=4)and(col_red<=4)and (row_red>=0)and (col_red>=0):
                    if(len(approx_red)==4):
...
#approx,moments for blue
    ...
    '''
            ...
            if(M["m00"]!=0):
                ...
                if(row_blue<=4)and(col_blue<=4)and (row_blue>=0)and (col_blue>=0):
...
#approx,moments for yellow
    ...
    '''
            ...
            if(M["m00"]!=0):
                ...
                if(row_yellow<=4)and(col_yellow<=4)and (row_yellow>=0)and (col_yellow>=0):
    ...
    for cnt in cnt_green:
        ...
        if area>50:
            ...
            if(M["m00"]!=0):
                ...
                if(row_green<=4)and(col_green<=4)and (row_green>=0)and (col_green>=0):
    ...
    '''
            ...
            if(M["m00"]!=0):
                ...
                if(row_white<=4)and(col_white<=4)and (row_white>=0)and (col_white>=0):
...
blue_bgr=frame[int(roi_blue[1]):int(roi_blue[1]+roi_blue[3]),int(roi_blue[0]):int(roi_blue[0]+roi_blue[2])]
...
while (count_horuxes!=0):
    ...
    while True:
        ...
        for x in white_box:
            for y in green_matrix:
                print('finding match green and white for destination')
                if(x==y):
                    break
            if(x==y):
                break
    ...
    path=djikstras(current_pos,x,array,extension=0,min_colour=0)
    ...
    while True:
        ...
        if(len(blue_destination)!=0):
            break
    _,current_pos,_=aruco_marker(frame,roi)
    path=djikstras(current_pos,blue_destination[0],array,extension=0,min_colour=7)
    pathFolllower(frame,path,current_pos,roi)
    ser.write(b'A')
    time.sleep(5)
    ser.write(b'P')
    ...
    if(count_horuxes==0):
        ...
        for x in white_box:
            for y in green_matrix:
                print('finding match green and white to see if ps is solved')
                if(x==y):
                    count_horuxes+=1
                    break
            if(x==y):
