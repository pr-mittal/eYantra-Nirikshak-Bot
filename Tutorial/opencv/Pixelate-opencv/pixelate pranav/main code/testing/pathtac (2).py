...
def djikstras(current_pos,final_pos,input_array,extension=0,min_colour=1):
    ...
    def dspa(a,b,c,d,e):
        if a!=-1 or b!=-1 or a!=5 or b!=5:
            if d_arr[a][b]>c:
                if arr[a][b]!=min_colour:
                    d_arr[a][b]=c+1
                    p_arr[a][b]=d
                    v_arr[a][b]=e+1
                elif arr[a][b]==min_colour:
                    d_arr[a][b]=c
                    p_arr[a][b]=d
                    v_arr[a][b]=e+1
        return arr
    ...
    def start(si,sj):
        dspa(si+1,sj,d_arr[si][sj],10*(si)+sj,e)
        dspa(si,sj+1,d_arr[si][sj],10*(si)+sj,e)
        dspa(si-1,sj,d_arr[si][sj],10*(si)+sj,e)
        dspa(si,sj-1,d_arr[si][sj],10*(si)+sj,e)
...
'''
thresh_value=np.array([[[white_bmin,white_gmin,white_rmin],[blue_bmin,blue_gmin,blue_rmin],[green_bmin,green_gmin,green_rmin],[yellow_bmin,yellow_gmin,yellow_rmin],[red_bmin,red_gmin,red_rmin]]]+[[[white_bmax,white_gmax,white_rmax],[blue_bmax,blue_gmax,blue_rmax],[green_bmax,green_gmax,green_rmax],[yellow_bmax,yellow_gmax,yellow_rmax],[red_bmax,red_gmax,red_rmax]]])
...
while (count_horuxes!=0):
    ...
    path=djikstras(current_pos,blue_destination[0],colour_matrix,extension=0,min_colour=1)
    pathFolllower(frame,path,current_pos,roi)
    ser.write(b'R')
    time.sleep(5)
    ser.write(b'P')
